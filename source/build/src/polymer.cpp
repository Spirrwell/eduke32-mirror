...
void                polymer_inb4rotatesprite(int16_t tilenum, char pal, int8_t shade, int32_t method)
{
    ...
    buildgl_bindSamplerObject(0, PTH_CLAMPED | ((rotatespritematerialbits & (1 << PR_BIT_ART_MAP)) ? PTH_INDEXED : 0));
...
//     overridematerial = 0;
        ...
        for (int z = bunchfirst[closest]; z >= 0; z = bunchp2[z])
        {
            ...
            if (((lightidx || depth) || (qstate[widx] & (Q_OCCLUDED|Q_DRAWN)) == 0) && wallvisible(globalposx, globalposy, wallnum))
            {
                    ...
                    || ((prwalls[wallnum]->underover & 2) && polymer_planeinfrustum(prwalls[wallnum]->over, frustum)))
                {
                    ...
                    if (!depth && (qstate[widx] & Q_VISIBLE) && (overridematerial & (1 << PR_BIT_MIRROR_MAP))
    ...
    while (front != back)
    {
        ...
        do
        {
            ...
            if (!depth && (qstate[i] & Q_VISIBLE) && (overridematerial & (1 << PR_BIT_MIRROR_MAP))
                && pr_wireframe == 0 && wal->overpicnum == 560 && (wal->cstat & CSTAT_WALL_1WAY))
